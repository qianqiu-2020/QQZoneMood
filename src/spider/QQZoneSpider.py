...
class QQZoneSpider(BaseSpider):
    ...
    def login(self):
        # 手动添加cookie
        self.cookie_text = self.re.get('cookies')
        if self.cookie_text:
    ...
    def login_with_qr_code(self):
        ...
        # self.init_user_info()
        # 已废弃self.get_qzone_token()

        # 不可直接转为字符串self.cookie_text = self.re.set('cookies',cookies.)
        if self.use_redis:
    ...
    def get_g_tk(self):
        ...
        for i in range(0, len(p_skey)):
            hash += (hash << 5) + self.utf8_unicode(p_skey[i])
    ...
    # 获取动态详情列表
    def get_mood_list(self, current_friend):
        """
        ...
        print("说说总数" + str(mood_num))
        already_num = int(self.re.get(current_friend))  # 这里如果不在redis中设置初值，则可能会报错？？
        # 如果mood_num被设定为-1，则自动增量爬取
        if self.mood_num == -1:
            # 已爬数量大于等于总数，则说明已经全部爬取，不用再爬取了
            if already_num >= mood_num:
                return
            # 否则再次爬取
            self.mood_num = mood_num - already_num
        # 否则就是设定值本身

        print("将要下载的说说数量为" + str(self.mood_num))
        ...
        # 记录当前已经爬取的数目
        self.re.set(current_friend, already_num + self.mood_num)
        self.result_report()
        ...
        while pos < mood_num and until_stop_time:
            ...
            try:
                try:
                    ...
                    if not self.from_client:
                        # 获取每条动态的unikey,unikey里面有20条说说的详细内容，不只是key
    ...
    def do_get_infos(self, unikeys, until_stop_time):
        for unikey in unikeys:
            ...
            # 在这里实现增量爬取，判断是否已经爬过，如果已经爬到，说明再往前的所有说说也已经爬过，可以停止当前线程。
            if not self.re.sadd("has_crawled_unikeys", key):
                # 表示已经存在，则直接返回
                return False
