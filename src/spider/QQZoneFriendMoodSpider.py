...
class QQZoneFriendMoodSpider(QQZoneSpider):
    ...
    def __init__(self, use_redis=False, debug=False, analysis=True, recover=False,
        ...
        QQZoneSpider.__init__(self, use_redis=use_redis, debug=debug, username=username,
                              ...
                              recover=recover, pool_flag=pool_flag)
    ...
    def get_friend_mood(self, friend_qq='', nick_name='佚名', mood_num = -1,login_method =2):
        """
        ...
        for friend in self.friend_name_list:
            ...
            self.init_parameter()
            self.init_file_name()
            # 爬取数据，并保存到redis与本地json文件(目前修改为只存放在redis中)中
            self.get_mood_list(friend['friend_qq'])
...
if __name__ == '__main__':
    qqfriend = QQZoneFriendMoodSpider(use_redis=True, debug=False, mood_begin=0, mood_num=-1,
                                      ...
                                      download_mood_detail=True, download_like_detail=True, download_like_names=True,
                                      recover=False)
    qqfriend.get_friend_mood(mood_num=-1)
